# Deep structural analysis of APEX.wav.
# Outputs: tempo per 10-second window via flux autocorrelation,
# energy envelope summary (drops, quiet sections, climaxes),
# and beat-position estimates for major transitions.

$ErrorActionPreference = 'Stop'
$path = 'c:\Users\robbc\Rhythm\Music\APEX.wav'
...
Write-Output ("Total duration: " + [Math]::Round($totalSec, 2) + "s")
Write-Output ""
...
# ---- Build energy envelope at envFS Hz ----
$envFS = 100
...
# Spectral flux (positive differential)
...
# ---- Per-section BPM via autocorrelation in 10s windows ----
$winSec = 10
$winLen = $envFS * $winSec
$bpmLow = 60
$bpmHigh = 260
...
Write-Output "Per-section tempo (10s windows, autocorrelation of spectral flux):"
Write-Output ("Time      OverallEnergy   PeakBPM   2ndBPM   3rdBPM")
...
for ($w = 0; $w + $winLen -le $envLen; $w += [int]($envFS * 5)) {
  ...
  Write-Output $line
  $globalSummaries += [pscustomobject]@{ Time = $tStr; TimeSec = $tSec; Energy = $eAvg; PeakBPM = $top3[0].BPM }
}

# ---- Find energy peaks/drops ----
Write-Output ""
Write-Output "Energy timeline (1-second average, normalized):"
...
# Major energy transitions (drops & lulls)
Write-Output ""
Write-Output "Major energy transitions (>50% change vs 4-second moving avg):"
